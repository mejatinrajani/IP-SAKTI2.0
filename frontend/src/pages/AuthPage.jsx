...
import { backendSignUp } from '../lib/api'
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (mode === 'signin') {
        ...
        // Use backend signup — auto-confirms email via Supabase Admin API
        try {
        ...
        if (signInErr) {
          // Signed up fine but sign-in failed for some reason
          setInfo('Account created! You can now sign in.')
          setMode('signin')
          setPassword('')
          return
        }
        // onAuthStateChange will fire → useEffect redirects to /evaluate
      }
    } catch (err) {
