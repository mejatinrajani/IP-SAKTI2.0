...
export default function CTA({ t }) {
  ...
  const { user } = useAuth()
  const target = user ? '/evaluate' : '/auth'
  return (
    <section className="relative bg-navy-900 py-28 overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-6 text-center">
        <Animate type="fadeUp">
          ...
          <h2 className="text-4xl lg:text-5xl font-bold text-white mb-6 leading-tight tracking-tight">{t.ctaHeading}</h2>
          <p className="text-navy-300 text-lg mb-12 leading-relaxed max-w-2xl mx-auto">{t.ctaPara}</p>
